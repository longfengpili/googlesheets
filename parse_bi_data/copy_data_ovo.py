...
class CopyDataOVO(ParseBiFunc):
    ...
    def repair_data_once(self, original_tablename, repair_tablename, n=1000, is_repair=True):
        ...
        logger_temp = repairbi_logger if is_repair else bakeup_logger
        #获取未修复数据
        # with lock:
    ...
    def copy_game_data_main(self, original_tablename, repair_tablename, id_min=None, id_max=None, n=1000, is_repair=True):
        ...
        copy_info = '修复' if is_repair else '复制'
        logger_temp = repairbi_logger if is_repair else bakeup_logger
        logger_temp.info(f'开始{copy_info}数据 ！ 【{self.db_host[:16]}】 to 【{(self.db2_host if self.db2_host else self.db_host)[:16]}】')
        self._connect()
        ...
        logger_temp.info(f'开始{copy_info}数据【({self.table2_id},{self.table_id}]】, 共【{counts}】条！')
        ...
        if counts == self.count:
            logger_temp.info(f'本次累计{copy_info}【({start_id},{self.table_id}]】, 共【{self.count}】条！')
        else:
            logger_temp.error(f'本次累计{copy_info}【({start_id},{self.table_id}]】, 预计【{counts}】条，实际【{self.count}】条！')
